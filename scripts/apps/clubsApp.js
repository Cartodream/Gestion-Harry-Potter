...
export class ClubsApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async #promptText(title, current = "") {
    return new Promise(resolve => {
      const d = new Dialog({
        title,
        content: `<input type="text" value="${current}" style="width:100%;margin-top:0.5rem" id="hp4-prompt-input">`,
        buttons: {
          ok: {
            label: "Valider",
            callback: html => resolve(html.find("#hp4-prompt-input").val().trim() || null)
          },
          cancel: { label: "Annuler", callback: () => resolve(null) }
        },
        default: "ok",
        render: html => setTimeout(() => html.find("#hp4-prompt-input").focus().select(), 50)
      });
      d.render(true);
    });
  }
